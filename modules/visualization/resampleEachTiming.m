...
[input arguments]
selected_file_name_list: [cell array], list of selected file name
...
function [ref_timing_EMG_struct] = resampleEachTiming(selected_file_name_list, ref_timing_EMG_struct, ref_timing, select_folder_path, element_num)  
    session_num = length(selected_file_name_list);
    for session_id = 1: session_num 
        % load Pdata
        load(fullfile(select_folder_path, selected_file_name_list{session_id}), 'each_timing_cutout_mean_EMG_struct');
        
        % store activity data around the timing of interest in 'data'
        ref_session_EMG_data = each_timing_cutout_mean_EMG_struct.(['timing' num2str(ref_timing) '_AVE']);
