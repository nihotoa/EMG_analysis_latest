...
function [AllData_EMG, TimeRange, EMG_Hz] = makeEasyEMG(monkey_prefix, xpdate, file_num, full_monkey_name, EMG_num)
%% create EMG All Data matrix
file_count = (file_num(end) - file_num(1)) + 1;
AllData_EMG_sel = cell(file_count,1);
load(fullfile(pwd, full_monkey_name, [monkey_prefix xpdate '-' sprintf('%04d',file_num(1,1))]),'CEMG_001_TimeBegin');
TimeRange = zeros(1,2);
...
for i = file_num(1,1):file_num(end)
    for j = 1:EMG_num
        if get_first_data
            load(fullfile(pwd, full_monkey_name, [monkey_prefix xpdate '-' sprintf('%04d',i)]), [EMG_prefix '_001*']);
            ...
            load(fullfile(pwd, full_monkey_name, [monkey_prefix xpdate '-' sprintf('%04d',i)]), [EMG_prefix '_0' sprintf('%02d',j)]);
